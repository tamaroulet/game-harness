...
- ハーネスが呼ぶすべてのエージェント（分解役・構造化役・実装役など）の設定に、`model_flag` と正確なモデルの ID を書く。CLI の既定のモデルには頼らない
- 実行のたびに、CLI が報告したモデルを記録し、設定と照合する。報告が無い・違う実行は止める（`harness/model_pin.py`。`python -m harness.model_pin` で設定を一斉に検査する）
