...
- 状態: 監査と改定の仕様。report.py の所要時間と総コスト・損益分岐、§5 の構造的な無駄の是正は実装済み（このファイルと同じ PR）。§4 の driver・pipeline の修正は、§6 の裁定の後に別 PR で入れる
...
- 「単一チャットの自然な運用（仕様の分解もテストも無い）に比べて、Layer 2 まで含めて元が取れるか」を問うなら、第 3 の条件（A0：要求文だけを渡し、生成テストは測定のときだけ置いて見せない）が要る。これは実験の問いを変えるので、§6 の裁定に回す
...
## 4. driver・pipeline の修正案（§6 の裁定の後に別 PR）
...
| F4 | 初回のプロンプトを 1 か所で作る。`pipeline.call_implementer` の前置きと本文の組み立てを `build_prompt(unit, workdir)` に切り出し、A もそれを使う。DISPUTE_TEST は §6 の裁定に従って両方に入れるか両方から外す | `pipeline.py`、`templates/initial.md` |
...
| F7 | `seconds_implementer`・`seconds_harness` を足す（`seconds_measure` は §5 で実装済み） | `driver.run_condition`・`run_task_b` |
...
## 5. 防壁型の構造的な無駄の点検と是正（dry-02）

### 5.1 入れた是正

| 是正 | 対象 | 根拠 |
|:--|:--|:--|
| 門の自己検査をタスクから外す（`--skip-selftest`） | `driver.pipeline_args` | dry-01 で pipeline 全体 405.9 秒のうち 177.1 秒。門の健全性は tests/ と `pipeline --selftest` で別に確かめる |
| 実装役にビルド・テスト・検証をさせない（外側の門に一元化）。編集したら直ちに終える | `pipeline.call_implementer` | 検証は内側ループと門が行い、失敗は次の呼び出しで渡している |
| `seconds` を条件の仕事（実装役と B の門）だけにし、測定器の秒を `seconds_measure` に分ける | `driver.run_condition` | 測定器は両条件に同じ実験の道具で、防壁型の費用ではない |
| 測定器の不変条件の 2 つ目以降のシードは `--no-build` | `measure.invariants` | シードは環境変数で渡すだけで、成果物は同じ |

### 5.2 点検の結果

1. **ドライバと pipeline の二重測定：ある。残す**。B のタスクの後、ドライバは高速検査の全件と不変条件（3 シード）をもう一度走らせる。これは A・B に同じ測定器を当てるための実験の道具で、B の門の結果を指標に使うと、門の無い A と測り方が変わる（設計 §2-3）。費用は `seconds_measure` に分けたので、B の費用には入らない（dry-02 で A 7.5 秒、B 7.6 秒）
2. **dotnet の多重起動**：B の 1 タスクで起動するのは、基準の確立（高速検査とエンジン）、内側ループのターンごとの高速検査、エンジンの受入（Unity の EditMode、8.7 秒）、不変条件 1 回。門ごとに問うものが違う（F2P・P2P、エンジン、保存則）ので、まとめられるものは無い。測定器の不変条件の再ビルドだけ省いた
3. **サンドボックスの I/O**：`sandbox_reset` は `git reset --hard`・`git clean -fd`・`git status` で、全ファイルのコピーはしない。ignored の `bin/`・`obj/` は残る。`purge_unwhitelisted_in_sandbox` も `git status` の結果の許可外だけを消す。無駄は無い。ただし自己検査を外したら、サンドボックスの冷えたビルドが基準の確立に移った（11.9 → 109.6 秒）。純減は自己検査の 177.1 秒ではなく、pipeline 全体で 107.7 秒（405.9 → 298.2）
4. **プロンプトの重複**：単位定義の `prompt` は 2,270〜2,914 字、`interface` の展開は各 1,500 字。dry-02 の B の入力 264,608 トークンに比べて小さく、入力の大半は実装役が自分の手番で積む文脈。削っても効かないので変えない

### 5.3 dry-02 の実測（T1、A・B を 1 回ずつ）

| 走行 | 条件 | 受入 | 秒 | うち pipeline の自己検査 / 基準の確立 / 試行 | 入力トークン | 出力トークン | キャッシュ読み |
|:--|:--|:--|:--|:--|:--|:--|:--|
| dry-01 | A | 10/10 | 184.2（測定を含む） | — | 224,628 | 24,966 | 未記録 |
| dry-01 | B | 10/10 | 414.9（測定を含む） | 177.1 / 11.9 / 215.8 | 624,444 | 28,153 | 1,285,060 |
| dry-02 | A | 10/10 | 178.7（＋測定 7.5） | — | 347,813 | 27,434 | 未記録 |
| dry-02 | B | 10/10 | 298.6（＋測定 7.6） | 省略 / 109.6 / 187.6 | 264,608 | 23,029 | 615,483 |

- 両条件とも P2P の破壊 0、不変条件の違反 0。外側の門は B の 1 試行で全段を通った（受入 8.7 秒、不変条件 4.9 秒）
- B の入力トークンは 624,444 → 264,608（57.6% 減）
- **1 回ずつなので、差を処置の効果とは言い切れない**。何も変えていない A が 224,628 → 347,813 と 1.5 倍に揺れた。揺らぎの幅は B4-RUN の繰り返しで見る
- 実装役にテストを走らせない指示は、B（pipeline）にだけ入った。A の初回テンプレートは凍結物（sha256）なので変えていない。これで F4 のプロンプトの差が 1 点増えた（裁定 2 に含める）
...
## 7. 裁定（2026-09-24、B4-RUN v1.0）と反映

| # | 裁定 | 反映 |
|:--|:--|:--|
| 1 | 呼び出しの予算をそろえる（F1） | A の上限を `max_attempts × MAX_INNER_LOOP_TURNS`（= 9）にした（`driver.call_budget`）。B の内側ループの全呼び出しを `implementer_calls` に積み、ドライバが合算する（F2） |
| 2 | 段階的にテストを解禁する（F4） | 文面を `harness/tool_policy.py` の 1 か所に置き、A・B の両方が使う。タスクの最初の呼び出しはビルド・テスト・探索を禁じ、失敗を返したあとの呼び出しでは解禁する。B では外側の再試行と、内側ループの 2 ターン目以降が「失敗を返したあと」にあたる。どちらも実装役に失敗を返してからの呼び出しで、A の再試行と同じ扱いにした |
| 2 | 再試行の知らせをそろえる | 落ちたテストの名前とアサーションの本文（期待値と実際の値）を、TRX から同じ関数で作る（`adapters/dotnet.failure_digest`）。これまで B の内側ループには名前だけが渡っていた。A はこれに加えて出力の末尾 40 行を受け取る（凍結したテンプレートの欄）。ビルドが通らないとき、B は診断の射影を受け取る（B の処置） |
| 3 | A0 は見送る | — |
| 4 | キャッシュ読みを含めた実コストで比べる | ドライバが `cache_read` を記録する（F6）。集計表に、キャッシュ読み・総入力（入力 + キャッシュ読み）・USD の列を足した。単価は `experiments/b4_ab/cost_model.json` |
| 5 | N=1 のスモークの後に 4 回 | `run-all --repeat 1 --prefix b4-smoke` の後、`--repeat 4` |

残る差：DISPUTE_TEST の逃げ道は B にだけある（裁定の対象外）。A の打ち切りの基準（F3）は変えていない。
