...
#### `required_symbols` の照合規則

静的な門（`harness/pipeline.py` の `symbol_found`）は 2 通りで照合する。どちらになるかは**書き方で決まる**。

| 書き方 | 例 | 照合 |
|:--|:--|:--|
| ドットで繋がった識別子だけ（**修飾名**） | `GameState.Width` | その並びがそのまま在るか、**または**ドットで分けた各識別子（`GameState` と `Width`）が語として在るか |
| それ以外（コード片を含む） | `class MetaPointRules`、`[SerializeField] private int _turnBonusPerTurn` | 生の部分一致（今までどおり） |

修飾名を分けて見るのは、**宣言側のソースにはその並びが現れない**ため。C# なら `class GameState` の中に
`public int Width` と書くので、`GameState.Width` を生の部分一致で探すと、実装が正しくても必ず落ちる
（2026-09-18 の Issue #12 が 3 試行ともこれで REJECT になった。実装役が回した受入テストは 37 件すべて通っていた）。

コード片を正規表現として解釈しないのは、`[SerializeField]` が文字クラスになって壊れるため。
修飾名と見なすのは `[A-Za-z_]\w*(\.[A-Za-z_]\w*)+` に**全体が**一致するものだけで、空白や記号を含むものは対象外。

**この門が測っていないもの**: その要素が本当にその型の中で宣言されているかは見ない
（`GameState.Width` は、別の型が `Width` を持っていても通る）。受入テストの前に置く粗い篩であって、
正解を決めるのは分解役が書いたテストである。厳密にしたいなら、ここではなくテスト側で縛る。
